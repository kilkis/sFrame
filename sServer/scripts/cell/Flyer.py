...
from interfaces.SkillMotion import SkillMotion

class Flyer(KBEngine.Entity, GameObject, SkillMotion):
	def __init__(self):
	...
	def onTimer(self, tid, userArg):
		if not self.init:
			self.init = True
			#增加范围监测
			self.territoryControllerID = self.addProximity(3, 0, 0)
			
		result = False
		self.life = self.life - 0.1
		ERROR_MSG("flyer life:%f"%(self.life))
		if self.life <= 0:
			result = True
		...
		if result == True:
			#生命周期结束或者次数满足后去除范围监控
			ERROR_MSG("flyer nums:%i"%(len(self.trapEntity)))
